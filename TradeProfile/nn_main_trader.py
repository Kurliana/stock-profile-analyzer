...
class NNTrader():
    ...
    def __init__(self, model_path = "nn_trade_test_model.h5", init_model = True):
        ...
        if init_model:
            self.prepare_data("FEES_1.txt.",mode="rus")
            self.init_model()
        else:
    ...
    def init_model(self):
        ...
        self.model.compile(loss=keras.losses.categorical_crossentropy,
                      optimizer='adam',
                      metrics=['accuracy'])
        self.model.fit(self.x_test, self.y_test, epochs=20, batch_size=128, verbose=1)   
    ...
    def prepare_data(self,data_file,mode="rus"):
        ...
        for ind in range(len(self.pa.tickers)): 
            if ind > 5 and ind < len(self.x_total)-6: #and ind < len(x_total)*5/10:
                ...
                self.x_test[-1]+=self.pa.best_directions[ind-5:ind]+[0]#+pa.best_directions[ind+1:ind+3]
    ...
    def get_last_direction(self):
        ...
        for ind in range(len(self.pa.tickers)):
            ...
            if ind >= 6:
                ...
                x_check[-1]+=new_best_directions[-5:]+[0]
        ...
        day_ranges={0:[100000, 10000, 100000,  230000, 1, 0, 0.04, 1, 'simple_0.04', 9, 20],
...
if __name__ == "__main__":
    nnt = NNTrader()
    nnt.prepare_data("C:\\Just2Trade Client\\tickers_CATERPILLAR Inc.txt","world")
    print nnt.get_last_direction()