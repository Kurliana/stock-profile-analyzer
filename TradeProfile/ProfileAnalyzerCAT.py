...
#            return -1
    ...
    def get_day_profit_old(self, curr_date, period = 30):
        ...
        for best_range in best_ranges:
            begin_time,check_time,start_time,end_time = best_range[0], best_range[1], best_range[2], best_range[3]
            day_profit, day_count, day_procent, day_list_profit, is_up_volume = self.analyze_by_day(period_day_tickers, check_time, start_time, end_time, 0, 0.015, 0.015, best_range[4], 200, True)
            log.info("Period: day_profit %s, day_count %s, day_procent %s, isup volume %s" % (day_profit, day_count, day_procent,is_up_volume))
            log.info("Begin %s, check %s, start %s, end %s,trade direct %s" % (begin_time,check_time,start_time,end_time,best_range[4]))
            if day_count < 2:
                day_tickers = self.filter_tickers(self.tickers, begin_time,end_time,curr_date,curr_date)
...
if __name__ == "__main__":
    ...
    if not pa.get_best_ranges_profit_std_extra_limits2(results_days_dir, results_profit,8,0.6,5,-5) or not pa.get_best_ranges_profit_std_extra_limits(results_days_dir, results_profit,8,0.6,5,-5):
        ...
        begin_time,check_time,start_time,end_time,trade = pa.get_best_ranges(results_days_dir, results_profit_dir,8,0.6,-5)[0]        
        period_day_tickers = pa.filter_tickers(pa.tickers, 94000,160000,pa.days[-6],pa.days[-1])
        day_profit, day_count, day_procent, day_list_profit, is_up_volume = pa.analyze_by_day(period_day_tickers, check_time, start_time, end_time, 0, 0.0015, 0.015, 1, 0.01, True)
        if day_count < 2:
            with open(result_file, 'wb') as f:
