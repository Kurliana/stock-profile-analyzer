        ...
        for ticker_id in range(len(tickers_list)):
            ...
            if ticker[3] >= start_time and ticker[3] <= end_time:
                ...
                if not total_ticker:
                    ...
                    #log.info(ticker)
                    if (ticker[3] < end_time) and ((take == 0) or (direction > 0 and ticker[6] < ticker[9]["VWMA%d" % ema1]) or (direction < 0 and ticker[5] > ticker[9]["VWMA%d" % ema1])):
                        #log.info("%s Start time %s value %s high %s atr %s" % (ticker[2],ticker[3],ticker[4],ticker[7],ticker[9]["ATRTS"+str(take)]))
                        #lets_enter+=1
                        #if lets_enter > 1 or ticker[3] == start_time:
                        total_ticker+=ticker
...
#                if ema1==ema2 and ema1 == 0:
    ...
    def analyze_by_day(self, tickers, check_time=102000, start_time=102000, end_time=120000, delta=0, direction_delta = 0.001, stop_loss = 0.015, reverse_trade=1, take_profit = 200, ignore_check_limit = True, take_profit_schema = "Noschema", ema1 = 0,ema2 = 0):
        ...
        for single_ticker in tickers:
            if single_ticker[2] == day_tickers[0][2]:
                ...
                #    is_up = self.is_up_direction_old(ticker1,check_time,direction_delta)*reverse_trade
                #else:
                is_up = self.is_up_direction4(day_tickers,-1,check_time,direction_delta,ema1,ema2)*reverse_trade+self.is_up_direction3(day_tickers,-1,check_time,direction_delta,ema1,ema2)*reverse_trade
                #is_up = self.is_up_direction(day_tickers,-1,check_time,take_profit)*reverse_trade
                if not is_up == 0:
                    #ticker2=self.combine_multi_tickers(day_tickers,start_time,end_time)
    ...
    def get_ranges_by_dayweek_new(self,curr_date):
        day_of_week = self.get_day_week(curr_date)
        day_ranges={0:[100000, 104000, 111000, 180000, 1, 0, 0.01, 1, 'take_innsta_0.04', 27, 0],
                    1:[100000, 115000, 125000, 170000, 1, 0, 0.04, 1, 'take_innsta_0.01', 9, 14],
...
if __name__ == "__main__":
    ...
    #for i in pa.tickers:
    #    if i[2]==20171113:
    #    log.info(i[:-1]+[i[9]["PVVREL"]])
    #log.info(pa.robot(-1,5,delta=0.0015,loss=0.03,methods_list=[0,2,4,6,8,10,12],best_prof=0,max_prof=1000,changer_period=5))
